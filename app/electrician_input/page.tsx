...
function ElectricianInput() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // Build the new electrician object; trade is set to "electrician"
    ...
    try {
      const res = await fetch("/api/services", {
        ...
        body: JSON.stringify(newElectrician),
      });
      const json = await res.json();
      if (json.success) {
        setConfirmationMessage("Electrician service listed successfully!");
        console.log("Electrician service successfully added!", newElectrician);
        router.push("/profile"); // Redirect to profile after submission
      } else {
        setConfirmationMessage("Error creating electrician service: " + json.error);
      }
    } catch (error) {
      console.error("Error creating electrician service:", error);
      setConfirmationMessage("Error creating electrician service, please try again later.");
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center" style={{ backgroundColor: "#f5d9bc" }}>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded shadow-md w-full max-w-2xl space-y-4">
        ...
        {/* License Information */}
        <div>
          <label className="block text-black mb-2">License Information</label>
          <input
            type="text"
            value={license}
...
export default function ElectricianInputPage() {
  return (
    <SessionProvider>
      <ElectricianInput />
    </SessionProvider>
